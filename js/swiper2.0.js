var swiper = (function () {
    ...
    return {
        init(ele) {
            ...
            // 获取上一页按钮
            $prevBtn = obj.querySelector('.prev');
            $nextBtn = obj.querySelector('.next');
            // 获取下一页按钮
            $bannerBox = obj.firstElementChild;
            $tipBox = $bannerBox.nextElementSibling;//left
            $tipAll = document.querySelectorAll('.uu li');
        ...
        showImage() {
            ...
            for(let i = 0; i < $tipAll.length; i++) {
                $tipAll[i].classList.remove('li-js');
                // $tipAll[i].className.replace('active', '');

            }
            $tipAll[index].classList.add('li-js');
